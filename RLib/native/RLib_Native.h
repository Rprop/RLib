...
// nullptr terminated, Length does not include trailing NULL.
//
...
typedef NTSTRING ANSI_STRING;
typedef PNTSTRING PANSI_STRING;

typedef NTSTRING OEM_STRING;
typedef PNTSTRING POEM_STRING;
typedef CONST NTSTRING* PCOEM_STRING;

typedef const UNICODE_STRING *PCUNICODE_STRING;

#define NT_UNICODE_NULL ((wchar_t)0) // winnt
...
#define INIT_UNICODE_STRING(us, wch)   \
  ...
  us.Length    = (USHORT)(wcslen(wch) * sizeof(wchar_t)); \
  us.Buffer    = wch
#endif
...
NTSYSAPI NTSTATUS NTAPI RtlInitializeCriticalSectionAndSpinCount(RTL_CRITICAL_SECTION *crit, ULONG spincount);
...
typedef struct _KEY_FULL_INFORMATION
{
  ...
  wchar_t Class[1];  // Variable length
...
// entry is interpreted.  A nullptr name indicates the end of the table.
//
...
#define RTL_QUERY_REGISTRY_DIRECT 0x00000020  // QueryRoutine field ignored.  EntryContext
 ...
 // If .Buffer field is nullptr then a buffer is
 // allocated.
 //
...
NTSTATUS
NTAPI
ZwCreateThreadEx(OUT PHANDLE ThreadHandle,
				 IN ACCESS_MASK DesiredAccess,
				 IN POBJECT_ATTRIBUTES ObjectAttributes OPTIONAL,
				 IN HANDLE ProcessHandle,
				 IN PTHREAD_START_ROUTINE StartRoutine,
...
VOID
NTAPI
RtlUserThreadStart(IN PTHREAD_START_ROUTINE pvStartRoutine,
				 IN PVOID pvParam);
...
/*++
  ...
  ObjectAttributes - Points to a structure that specifies the object抯
...
NTSYSAPI VOID NTAPI RtlExitUserThread(ULONG status);
NTSYSAPI VOID NTAPI RtlExitUserProcess(ULONG status);
...
NTSYSAPI NTSTATUS NTAPI RtlHashUnicodeString(PCUNICODE_STRING string, BOOLEAN case_insensitive, ULONG alg, ULONG *hash);
NTSYSAPI NTSTATUS NTAPI NtCreateSemaphore(OUT PHANDLE SemaphoreHandle, IN ACCESS_MASK DesiredAccess, IN POBJECT_ATTRIBUTES ObjectAttributes OPTIONAL, IN ULONG InitialCount, IN ULONG MaximumCount);
...
NTSYSAPI NTSTATUS NTAPI NtReleaseSemaphore(IN HANDLE SemaphoreHandle, IN ULONG  ReleaseCount, OUT PULONG  PreviousCount OPTIONAL);

NTSYSAPI NTSTATUS NTAPI NtCreateMutant(OUT PHANDLE MutantHandle, IN ACCESS_MASK DesiredAccess, IN POBJECT_ATTRIBUTES ObjectAttributes OPTIONAL, IN BOOLEAN InitialOwner);
NTSYSAPI NTSTATUS NTAPI NtReleaseMutant(IN HANDLE MutantHandle, OUT PLONG PreviousCount OPTIONAL);
...
NTSYSAPI NTSTATUS NTAPI NtQueryFullAttributesFile(__in POBJECT_ATTRIBUTES ObjectAttributes, __out PFILE_NETWORK_OPEN_INFORMATION FileInformation);
...
NTSYSAPI NTSTATUS NTAPI NtSetInformationThread(IN HANDLE ThreadHandle, IN THREADINFOCLASS ThreadInformationClass, IN PVOID ThreadInformation, IN ULONG ThreadInformationLength);
NTSYSAPI NTSTATUS NTAPI NtAlertThread(HANDLE ThreadHandle);
NTSYSAPI NTSTATUS NTAPI NtAlertResumeThread(IN HANDLE ThreadHandle,OUT PULONG  SuspendCount );
...
#pragma warning(default: 4200 4201 4005)
